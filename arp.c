...
void refresharp(ARPTAB_ENTRY* list) {
  if (debug) printf("Refreshing ARP entries.\n");
...
void rq_process(struct in_addr ipaddr, int ifindex) {
  ...
  while (cur_entry != NULL) {
    if (ipaddr.s_addr == *((long*)cur_entry->req_frame.arp.arp_tpa) &&
        ifindex != cur_entry->req_if.sll_ifindex) {
      if (debug) printf("Found %s in request queue\n", inet_ntoa(ipaddr));
      arp_reply(&cur_entry->req_frame, &cur_entry->req_if);

      /* Delete entry from the linked list */

      if (cur_entry == req_queue_tail) req_queue_tail = prev_entry;
...
void* arp(char* ifname) {
  ...
  /* TODO: FIXME properly (how should we do cancelling without pthread cancel
   * support?) */
  ...
  while (1) {
    ...
    do {
      ...
      if (arp_recv(sock, &frame) <= 0) continue;
      ...
      if (frame.arp.arp_op == ntohs(ARPOP_REPLY)) {
        ...
        if (option_arpperm) k_arpreq.arp_flags = k_arpreq.arp_flags | ATF_PERM;
        ...
        if (debug)
          printf(
              "Received reply: updating kernel ARP table for "
              "%s(%s).\n",
        ...
        /* send gratuitous arp request to all other interfaces to let
         * them update their ARP tables quickly */
