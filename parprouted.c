...
ARPTAB_ENTRY *replace_entry(struct in_addr ipaddr, char *dev) {
    ...
    if (cur_entry == NULL) {
        ...
        if ((cur_entry = (ARPTAB_ENTRY *)malloc(sizeof(ARPTAB_ENTRY))) ==
            NULL) {
    ...
    while (!feof(arpf)) {
        ...
        if (fgets(line, ARP_LINE_LEN, arpf) == NULL) {
            ...
            if (incomplete & !findentry(ipaddr)) {
                if (debug)
                    printf("incomplete entry %s found, request on all "
                           "interfaces\n",
                           inet_ntoa(ipaddr));
            ...
            if (entry->want_route) {
                if (remove_other_routes(entry->ipaddr_ia, entry->ifname) > 0)
                    if (debug)
                        printf("Found ARP entry %s(%s), removed entries via "
                               "other interfaces\n",
            ...
            if (debug && !entry->route_added && entry->want_route) {
                printf("arptab entry: '%s' HWAddr: '%s' Dev: '%s' "
                       "route_added:%d want_route:%d\n",
                       inet_ntoa(entry->ipaddr_ia), entry->hwaddr,
                       entry->ifname, entry->route_added, entry->want_route);
            }
        }
    }
