...
int route_remove(ARPTAB_ENTRY* cur_entry) {
  ...
  if (success) cur_entry->route_added = 0;
...
int route_add(ARPTAB_ENTRY* cur_entry) {
  ...
  if (success) cur_entry->route_added = 1;
...
void processarp(int in_cleanup) {
  ...
  while (cur_entry != NULL) {
    ...
    if (!cur_entry->want_route ||
        ...
        in_cleanup) {
      if (cur_entry->route_added) route_remove(cur_entry);
...
void parseproc() {
  ...
  while (!feof(arpf)) {
    if (fgets(line, ARP_LINE_LEN, arpf) == NULL) {
      ...
      if (strstr(line, "00:00:00:00:00:00") != NULL) incomplete = 1;

      /* Incomplete entries having flag 0x0 */
      if (strstr(line, "0x0") != NULL) incomplete = 1;
      ...
      /* if IP address is marked as undiscovered and does not exist in
         arptab, send ARP request to all ifaces */

      if (incomplete & !findentry(ipaddr)) {
        if (debug)
          printf(
              "incomplete entry %s found, request on all "
              "interfaces\n",
              inet_ntoa(ipaddr));
        for (i = 0; i <= last_iface_idx; i++) arp_req(ifaces[i], ipaddr, 0);
      }
      ...
      if (entry->want_route) {
        if (remove_other_routes(entry->ipaddr_ia, entry->ifname) > 0)
          if (debug)
            printf(
                "Found ARP entry %s(%s), removed entries via "
                "other interfaces\n",
      ...
      if (debug && !entry->route_added && entry->want_route) {
        printf(
            "arptab entry: '%s' HWAddr: '%s' Dev: '%s' "
            "route_added:%d want_route:%d\n",
